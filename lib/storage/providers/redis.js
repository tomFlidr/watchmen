...
StorageRedis.prototype.addService = function (service, callback) {
  ...
  multi.set(SERVICE_KEY_SUFIX + ':' + id, stringify(service));
...
StorageRedis.prototype.updateService = function (service, callback) {
  var self = this;
  var multi = this.redis.multi();
  multi.set(SERVICE_KEY_SUFIX + ':' + service.id, stringify(service));
  multi.exec(function (err) {
    if (err) {
      return callback(err);
    }
...
StorageRedis.prototype.startOutage = function (service, outageData, callback) {
  this.redis.set(service.id + ':' + CURRENT_OUTAGE_KEY_SUFIX, stringify(outageData), function (err) {
    callback(err);
  });
};
...
StorageRedis.prototype.archiveCurrentOutageIfExists = function (service, callback) {
  ...
  this.getCurrentOutage(service, function (err, outage) {
    ...
    if (outage) {
      ...
      multi.zadd(service.id + ':' + OUTAGES_KEY_SUFIX, outage.timestamp, stringify(outage));
      multi.exec(function (err) {
        callback(err, outage);
      });
    } else {
