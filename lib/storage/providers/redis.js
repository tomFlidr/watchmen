...
StorageRedis.prototype.startOutage = function (service, outageData, callback) {
  var outageDataStr = '';
  try {
	  outageDataStr = JSON.stringify(outageData);
  } catch (e) {
	  outageDataStr = stringify(outageData);
  }
  this.redis.set(service.id + ':' + CURRENT_OUTAGE_KEY_SUFIX, outageDataStr, function (err) {
    callback(err);
  });
};
...
StorageRedis.prototype.archiveCurrentOutageIfExists = function (service, callback) {
  ...
  this.getCurrentOutage(service, function (err, outage) {
    ...
    if (outage) {
	  ...
	  var outageDataStr = '';
	  try {
		  outageDataStr = JSON.stringify(outage);
	  } catch (e) {
		  outageDataStr = stringify(outage);
	  }
      multi.zadd(service.id + ':' + OUTAGES_KEY_SUFIX, outage.timestamp, outageDataStr);
      multi.exec(function (err) {
        callback(err, outage);
      });
    } else {
