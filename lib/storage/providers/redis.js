...
StorageRedis.prototype.startOutage = function (service, outageData, callback) {
  this.redis.set(service.id + ':' + CURRENT_OUTAGE_KEY_SUFIX, CircularJSON.stringify(outageData), function (err) {
    callback(err);
  });
};
...
StorageRedis.prototype.archiveCurrentOutageIfExists = function (service, callback) {
  ...
  this.getCurrentOutage(service, function (err, outage) {
    ...
    if (outage) {
      ...
      multi.zadd(service.id + ':' + OUTAGES_KEY_SUFIX, outage.timestamp, CircularJSON.stringify(outage));
      multi.exec(function (err) {
        callback(err, outage);
      });
    } else {
